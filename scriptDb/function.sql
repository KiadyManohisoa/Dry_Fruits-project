...
--query test 
select * from get_Balance_By_Date('2024-06-13',2);
...
SELECT
    ...
    JOIN
        products_ordered po ON o.id_order = po.id_order
    LEFT JOIN
        detail_movement dm ON po.id_product = dm.id_product AND po.sales_type = 'D' AND dm.movement_date = (select max(movement_date) from detail_movement where DATE(movement_date) <='2024-06-13')
    LEFT JOIN
        wholesale_movement wm ON po.id_product = wm.id_product AND po.sales_type = 'W' AND wm.movement_date = (select max(movement_date) from wholesale_movement where DATE(movement_date) <='2024-06-13')
    LEFT JOIN
        bulk_movement bm ON po.id_product = bm.id_product AND po.sales_type = 'B' AND bm.movement_date = (select max(movement_date) from bulk_movement where DATE(movement_date) <='2024-06-13')
    JOIN
        Product p ON po.id_product = p.id_product
    WHERE
        p.id_cat_product= 1
        AND DATE(o.ordering_date) = '2024-06-13'
    GROUP BY
        p.id_cat_product,p.id_product; 
