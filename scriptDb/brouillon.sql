        select s.id_product, vpd.product_name, s.total_quantity - COALESCE(o.total_quantity,0) as stock, COALESCE(o.total_quantity,0) as out, COALESCE(sales.number_package,0) as sales, charges.price * (COALESCE(o.total_quantity,0)) as charges, COALESCE(sales_amount.total_price,0) as sales_amount ,COALESCE(sales_amount.total_price,0) - charges.price * (COALESCE(o.total_quantity,0)) as results from
            ...
            (SELECT
                    ...
                    JOIN
                        products_ordered po ON o.id_order = po.id_order
                    LEFT JOIN
                        detail_movement dm ON po.id_product = dm.id_product AND po.sales_type = ''D'' AND dm.movement_date = (select max(movement_date) from detail_movement where movement_date <=DATE(o.ordering_date))
                    LEFT JOIN
                        wholesale_movement wm ON po.id_product = wm.id_product AND po.sales_type = ''W'' AND wm.movement_date = (select max(movement_date) from wholesale_movement where movement_date <=DATE(o.ordering_date))
                    LEFT JOIN
                        bulk_movement bm ON po.id_product = bm.id_product AND po.sales_type = ''B'' AND bm.movement_date = (select max(movement_date) from bulk_movement where movement_date <=DATE(o.ordering_date))
                    JOIN
                        Product p ON po.id_product = p.id_product
                    WHERE
